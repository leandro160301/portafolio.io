...
async function addSymbolToDB(symbolData) {
    try {
        ...
        // Update local state
        const existingIdx = AppData.symbols.findIndex(s => s.ticker === symbolData.ticker);
        if (existingIdx !== -1) {
            AppData.symbols[existingIdx] = { ...symbolData, _id: symbolData.ticker };
        } else {
...
function renderSymbolsTable() {
    ...
    AppData.symbols.sort((a, b) => a.ticker.localeCompare(b.ticker)).forEach((sym, index) => {
        ...
        const priceDisplay = sym.currentPrice ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(sym.currentPrice) : '-';
        const convDisplay = sym.conversionRatio ? sym.conversionRatio : '-';
        tr.innerHTML = `
            ...
            <td>${priceDisplay}</td>
            <td>
                <button class="btn-icon" onclick="editSymbol('${sym.ticker}')" title="Editar">
                    <i class="fa-solid fa-pen-to-square"></i>
                </button>
                <button class="btn-icon delete" onclick="deleteSymbol(${index})" title="Borrar">
...
function renderOperationsTable() {
    ...
    if (sortedOps.length === 0) {
        ...
        sortedOps.forEach((op) => {
            ...
            const symbolData = AppData.symbols.find(s => s.ticker === op.ticker);
            // Conversion Ratio: prioritize stored op.ratio, then symbolData.conversionRatio
            const conversionRatio = op.ratio || (symbolData ? symbolData.conversionRatio : 0);

            // Underlying Qty: prioritize stored op.underlyingQty, then calc
            let underlyingQty = op.underlyingQty;
            if (!underlyingQty && conversionRatio > 0 && op.qty) {
                underlyingQty = (op.qty / conversionRatio).toFixed(2);
            }

            // USD Amount: prioritize stored op.usdAmount, then calc
            let usdAmount = op.usdAmount;
            if (!usdAmount && op.mep > 0 && op.amount) {
                usdAmount = (op.amount / op.mep);
            }
            ...
            const formattedUsd = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(usdAmount || 0);
            const displayUnderlying = underlyingQty ? parseFloat(underlyingQty).toFixed(2) : '-';
...
function renderDashboard() {
    const holdings = {}; // Ticker -> Cedear Qty (for legacy display mostly)
    const holdingsUnderlying = {}; // Ticker -> Underlying Qty
    const costBasisComp = {}; // Ticker -> Cost in Current Currency (for table display)
    const costBasisUSD = {}; // Ticker -> Net Invested USD (for P/L)

    // Init totals
    let totalInvestedComp = 0;

    // Process Operations sorted chronologically to track cost basis properly?
    // For simple "average cost" or "net invested", we can just sum buys and subtract proportional cost on sell?
    // Or simpler: Net Invested = Sum(Buys USD) - Sum(Sells USD).
    // Note: This "Net Invested" approach works well for P/L = CurrentVal - NetInvested. 
    // If I sold everything, NetInvested should ideally be 0 (or realized profit, but here we track *current* holdings).
    // If we use "Net Cash Flow", selling at profit reduces cost basis potentially below zero.
    // Better Approach for "Portfolio Performance":
    // Cost Basis of CURRENT shares.
    // We need to track it per share.

    // Let's do a pass to build state
    const portfolioState = {}; // Ticker -> { shares: 0, totalCostUSD: 0, totalCostARS: 0 }

    const sortedOps = [...AppData.operations].sort((a, b) => new Date(a.date) - new Date(b.date));

    sortedOps.forEach(op => {
        const ticker = op.ticker;
        if (!portfolioState[ticker]) portfolioState[ticker] = { shares: 0, totalCostUSD: 0, totalCostARS: 0 };

        const symbolData = AppData.symbols.find(s => s.ticker === ticker);

        // Determine Underlying Qty
        let uQty = op.underlyingQty;
        if (!uQty) {
            // Fallback calc
            const ratio = op.ratio || (symbolData ? symbolData.conversionRatio : 0);
            if (ratio > 0) uQty = op.qty / ratio;
            else uQty = 0; // cannot determine
        }
        uQty = parseFloat(uQty);

        // Determine USD Amount
        let usdAmt = op.usdAmount;
        if (!usdAmt) {
            if (op.mep > 0) usdAmt = op.amount / op.mep;
            else usdAmt = 0;
        }
        usdAmt = parseFloat(usdAmt);

        if (op.type === 'BUY') {
            portfolioState[ticker].shares += uQty;
            portfolioState[ticker].totalCostUSD += usdAmt;
            portfolioState[ticker].totalCostARS += parseFloat(op.amount);

            // Legacy holdings
            holdings[ticker] = (holdings[ticker] || 0) + op.qty;
        } else {
            // SELL
            // Reduce cost basis proportionally to shares sold
            const currentShares = portfolioState[ticker].shares;
            if (currentShares > 0) {
                const fraction = uQty / currentShares;
                // Prevent negative cost if rounding errors
                if (fraction >= 0.99) {
                    portfolioState[ticker].totalCostUSD = 0;
                    portfolioState[ticker].totalCostARS = 0;
                } else {
                    portfolioState[ticker].totalCostUSD -= (portfolioState[ticker].totalCostUSD * fraction);
                    portfolioState[ticker].totalCostARS -= (portfolioState[ticker].totalCostARS * fraction);
                }
            }
            portfolioState[ticker].shares = Math.max(0, currentShares - uQty); // avoid neg
            holdings[ticker] = Math.max(0, (holdings[ticker] || 0) - op.qty);
        }
    });

    // Populate the flattened maps for reference by existing code
    Object.keys(portfolioState).forEach(t => {
        holdingsUnderlying[t] = portfolioState[t].shares;
        // Logic for CostBasisComp depends on currentCurrency
        if (currentCurrency === 'USD') {
            costBasisComp[t] = portfolioState[t].totalCostUSD;
            totalInvestedComp += portfolioState[t].totalCostUSD;
        } else {
            costBasisComp[t] = portfolioState[t].totalCostARS;
            totalInvestedComp += portfolioState[t].totalCostARS;
        }
    });
    ...
    const allTickers = new Set([...AppData.symbols.map(s => s.ticker), ...Object.keys(portfolioState)]);

    // Track Portfolio Totals for Cards
    let portTotalMarketUSD = 0;
    let portTotalCostUSD = 0;

    // First pass to build data list
    let tempData = [];

    allTickers.forEach(ticker => {
        const state = portfolioState[ticker] || { shares: 0, totalCostUSD: 0 };
        const uQty = state.shares;

        // Display Qty: context depends. The table header says 'Cantidad Neta'
        // User is used to Cedears. We should probably keep showing Cedears in the main column
        // but maybe we can tooltip the underlying.
        // Actually, let's stick to showing Cedears count (holdings[ticker]) if we can,
        // IF the table column header implies it. But 'holdings[ticker]' might be desynched if ratios changed.
        // Let's use `holdings[ticker]` (which sums cedear quantity) for display if available, 
        // else 0.
        const qtyDisplay = holdings[ticker] || 0;

        const cost = costBasisComp[ticker] || 0; // Cost in selected currency
        const symbolConfig = AppData.symbols.find(s => s.ticker === ticker);
        const targetRatio = symbolConfig ? symbolConfig.ratio : 0;
        const currentPrice = symbolConfig ? (symbolConfig.currentPrice || 0) : 0;

        // Gain Loss Calculation (USD only for now)
        let gainLossUSD = 0;
        let gainLossPct = 0;
        let marketValUSD = 0;

        if (uQty > 0.001) {
            marketValUSD = uQty * currentPrice;
            portTotalMarketUSD += marketValUSD;
            portTotalCostUSD += state.totalCostUSD;

            gainLossUSD = marketValUSD - state.totalCostUSD;
            if (state.totalCostUSD > 0) {
                gainLossPct = (gainLossUSD / state.totalCostUSD) * 100;
            }
        }

        if (qtyDisplay !== 0 || symbolConfig) {
            tempData.push({
                ticker: ticker + (symbolConfig ? '' : ' (No Listado)'),
                qty: qtyDisplay, // Showing Cedears count
                cost: cost,
                ...
                hasPrice: currentPrice > 0 && uQty > 0
            });
        }
    });
    ...
    const totalDescEl = document.querySelector('.card.stat-card .stat-desc');
    const totalTitleEl = document.querySelector('.card.stat-card h3');

    if (currentCurrency === 'USD') {
        const pl = portTotalMarketUSD - portTotalCostUSD;
        const plPct = portTotalCostUSD > 0 ? (pl / portTotalCostUSD) * 100 : 0;
        const color = pl >= 0 ? 'var(--success)' : 'var(--danger)';
        const sign = pl >= 0 ? '+' : '';

        totalTitleEl.innerHTML = `Valor Portafolio <span style="font-size:0.8em; color:${color}">(${sign}${plPct.toFixed(2)}%)</span>`;
        totalMoneyEl.textContent = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(portTotalMarketUSD);
        totalDescEl.innerHTML = `<span style="color:${color}">${sign}${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(pl)}</span> vs costo`;
    } else {
        totalTitleEl.textContent = 'Capital Invertido';
        totalMoneyEl.textContent = new Intl.NumberFormat(currencyFormat, { style: 'currency', currency: currencyCode }).format(totalInvestedComp);
        totalDescEl.textContent = 'Costo histórico neto';
    }

    // New Card: Total Cost USD (Always valid if we have USD data, regardless of view mode?)
    document.getElementById('total-cost-usd').textContent = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(portTotalCostUSD);
    ...
    if (tempData.length === 0) {
        ...
        tempData.forEach(row => {
            ...
            let gpCell = '-';
            if (row.hasPrice && row.cost > 0) { // Only show GP if we have price and existing cost (avoid div0 or inf)
                const color = row.gainLossUSD >= 0 ? 'var(--success)' : 'var(--danger)';
                const sign = row.gainLossUSD >= 0 ? '+' : '';
                gpCell = `<span style="color:${color}; font-weight:500;">
                    ${sign}${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(row.gainLossUSD)}
                    <br><span style="font-size:0.8em">(${sign}${row.gainLossPct.toFixed(2)}%)</span>
                 </span>`;
            } else if (!row.hasPrice && row.qty > 0) {
                gpCell = '<span style="color:var(--text-secondary); font-size:0.8em;">Falta Precio</span>';
            }

            // If we are in ARS viewing mode, showing GP in USD in that column (header says GP USD)
...
function setupEventListeners() {
    ...
    document.getElementById('symbol-form').addEventListener('submit', (e) => {
        e.preventDefault();
        const tickerInput = document.getElementById('symbol-ticker');
        const ticker = tickerInput.value.trim().toUpperCase();
        const ratio = parseFloat(document.getElementById('symbol-ratio').value);
        const conversionRatio = parseFloat(document.getElementById('symbol-conversion').value);
        const currentPrice = parseFloat(document.getElementById('symbol-current-price').value);

        if (!ticker || isNaN(ratio) || isNaN(conversionRatio)) return;

        // If not in edit mode (ticker input not disabled), check for duplicates
        if (!tickerInput.disabled && AppData.symbols.find(s => s.ticker === ticker)) {
            alert('El símbolo ya existe.');
            return;
        }

        addSymbolToDB({ ticker, ratio, conversionRatio, currentPrice: currentPrice || 0 });
        resetSymbolForm();
    });

    // Cancel Symbol Edit
    const btnCancelSymbol = document.getElementById('btn-cancel-symbol');
    if (btnCancelSymbol) {
        btnCancelSymbol.addEventListener('click', () => {
            resetSymbolForm();
        });
    }
    ...
    document.getElementById('operation-form').addEventListener('submit', (e) => {
        ...
        // New Fields
        const ratio = parseFloat(document.getElementById('op-ratio').value);
        const underlyingQty = parseFloat(document.getElementById('op-underlying').value);
        const usdAmount = parseFloat(document.getElementById('op-usd').value);
    ...
    // Auto-fill listeners
    document.getElementById('op-symbol').addEventListener('change', (e) => {
        const customRatio = document.getElementById('op-ratio');
        const ticker = e.target.value;
        const sym = AppData.symbols.find(s => s.ticker === ticker);
        if (sym && sym.conversionRatio) {
            customRatio.value = sym.conversionRatio;
            calcUnderlying();
        } else {
            customRatio.value = '';
        }
    ...
    document.getElementById('op-qty').addEventListener('input', calcUnderlying);
    document.getElementById('op-ratio').addEventListener('input', calcUnderlying);

    document.getElementById('op-amount').addEventListener('input', calcUsdAmount);
    document.getElementById('op-mep').addEventListener('input', calcUsdAmount);

    function calcUnderlying() {
        const qty = parseFloat(document.getElementById('op-qty').value);
        const ratio = parseFloat(document.getElementById('op-ratio').value);
        if (qty && ratio && ratio > 0) {
            document.getElementById('op-underlying').value = (qty / ratio).toFixed(2);
        }
    }

    function calcUsdAmount() {
        const amount = parseFloat(document.getElementById('op-amount').value);
        const mep = parseFloat(document.getElementById('op-mep').value);
        if (amount && mep && mep > 0) {
            document.getElementById('op-usd').value = (amount / mep).toFixed(2);
        }
    }
...
window.editSymbol = function (ticker) {
    const sym = AppData.symbols.find(s => s.ticker === ticker);
    if (!sym) return;

    document.getElementById('symbol-ticker').value = sym.ticker;
    document.getElementById('symbol-ticker').disabled = true; // Lock ticker
    document.getElementById('symbol-ratio').value = sym.ratio;
    document.getElementById('symbol-conversion').value = sym.conversionRatio || '';
    document.getElementById('symbol-current-price').value = sym.currentPrice || '';

    // Change UI
    document.getElementById('btn-submit-symbol').innerHTML = '<i class="fa-solid fa-check"></i> Actualizar';
    document.getElementById('btn-cancel-symbol').style.display = 'flex';

    // Scroll
    document.getElementById('symbol-form').scrollIntoView({ behavior: 'smooth' });
};

function resetSymbolForm() {
    document.getElementById('symbol-form').reset();
    document.getElementById('symbol-ticker').disabled = false;
    document.getElementById('btn-submit-symbol').innerHTML = '<i class="fa-solid fa-plus"></i> Agregar';
    document.getElementById('btn-cancel-symbol').style.display = 'none';
}
...
window.editOperation = function (id) {
    ...
    // Fill new fields
    document.getElementById('op-ratio').value = op.ratio || '';
    document.getElementById('op-underlying').value = op.underlyingQty || '';
    document.getElementById('op-usd').value = op.usdAmount || '';

    // Trigger calcs implicitly? Or let user see stored values.
    // Better to let stored values stand. If they edit, listeners will recalc.
            ...
            if (type === 'symbols') {
                ...
                if (confirm("Importar CSV de Acciones reemplazará toda la lista actual. ¿Continuar?")) {
                    ...
                    for (const r of result) {
                        const s = {
                            ...
                            conversionRatio: parseFloat(r.conversionRatio) || 0,
                ...
                for (const r of result) {
                    const op = {
                        ...
                        ratio: parseFloat(r.ratio) || 0,
                        underlyingQty: parseFloat(r.underlyingQty) || 0,
